<template>
  <div>
    <v-toolbar flat color="black">
      <v-toolbar-title>Second Section</v-toolbar-title>
      <v-divider
        class="mx-2"
        inset
        vertical
...
<script>
  export default {
    ...
    methods: {
      initialize () {
        const vm = this;
        axios.get('admin/secondSection/index').then(response => {
      ...
      deleteItem (item) {
        ...
        axios.get('admin/secondSection/destroy/' + item.id).then(response => {
      ...
      save () {
        if (this.editedIndex > -1) {
          const vm = this;
          axios.post('admin/secondSection/update',vm.editedItem).then(response => {
          ...
          axios.post('admin/secondSection/store',this.editedItem).then(response => {
